...
const EditorSidebar: React.FC = () => {
  ...
  return <div className="w-80 h-full border-l bg-white p-4">
      <h2 className="text-xl font-bold mb-6">Ferramentas</h2>
      
      <div className="grid grid-cols-2 gap-2 mb-8">
        <Button variant={currentTool === 'select' ? 'default' : 'outline'} onClick={() => handleToolSelect('select')} className="o bot\xE3o s\xF3 exibe sua modal ao clicar no room E SEGURAR. mas tem q funcionar como toggle. um clique exibe a modal. outro clique some.\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n">
          Selecionar
        </Button>
        <Button variant={currentTool === 'pan' ? 'default' : 'outline'} onClick={() => handleToolSelect('pan')}>
          Mover Mapa
        </Button>
        <Button variant={currentTool === 'room' ? 'default' : 'outline'} onClick={() => handleToolSelect('room')} className="flex items-center space-x-2">
          <MapPin size={16} />
          <span>Nova Sala</span>
        </Button>
        <Button variant={currentTool === 'desk' ? 'default' : 'outline'} onClick={() => handleToolSelect('desk')} className="flex items-center space-x-2">
          <Edit size={16} />
          <span>Nova Mesa</span>
        </Button>
      </div>
      ...
      {selectedArea ? <div>
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4">
              <Button variant="destructive" onClick={handleDeleteArea} className="w-full flex items-center justify-center space-x-2">
                <Trash size={16} />
                <span>Excluir Área</span>
              </Button>
            </div>
          </div>
        </div> : <div className="text-center text-gray-400 mt-12">
